...
class TestFAISSCaching:
    ...
    def test_cache_key_generation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            matcher1 = VideoFrameMatcher.from_paths(
    ...
    def test_cache_key_changes_with_parameters(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Different weights (comparison_size no longer exists)
            matcher1 = VideoFrameMatcher.from_paths(
                str(video_path), str(images_path), str(output_path), weights=(1.0, 2.0, 2.0)
            )
            matcher2 = VideoFrameMatcher.from_paths(
                str(video_path), str(images_path), str(output_path), weights=(1.0, 1.0, 1.0)
            )
...
class TestIntelligentFallback:
    ...
    def test_no_repeat_guarantees_zero_duplicates_across_many_frames(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            selected_images = [
                data["selected"] for data in checkpoint.values() if data.get("selected") is not None
            ]
